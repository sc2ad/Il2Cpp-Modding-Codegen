            ...
            if (_asHeader)
            {
                ...
                if (type.Parent != null)
                {
                    ...
                    if (_asHeader && type.This.Namespace == "System" && type.This.Name == "ValueType")
                        ...
                        s.parentName = context.GetNameFromReference(type.Parent, ForceAsType.Literal, mayNeedComplete: true);
                }
                stateDict[type] = s;
        ...
        public void Serialize(IndentedTextWriter writer, ITypeData type)
        {
            ...
            if (_asHeader)
            {
                ...
                if (type.ImplementingInterfaces.Count > 0)
                {
                    ...
                    bool hasParent = !string.IsNullOrEmpty(s);
                    bool first = true;
                    foreach (var t in type.ImplementingInterfaces)
                    {
                        if (!first)
                            writer.Write(", ");
                        if (!first || hasParent)
                            ...
                            s += " : ";
                        writer.Write(t);
                        s += $"public {_context.GetNameFromReference(t, ForceAsType.Literal, mayNeedComplete: true)}";
                        first = false;
                    }
                    writer.WriteLine();
                }
