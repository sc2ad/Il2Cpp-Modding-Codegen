...
{
    public class CppFieldSerializer : ISerializer<DllField, CppTypeWriter>
    {
        private readonly struct State
        {
            ...
            public State(string tn, string n, int s)
            {
                typeName = tn;
                name = n;
                size = s;
        ...
        private readonly ConcurrentDictionary<DllField, State> stateMap = new();

        public void Resolve(CppContext context, DllField t)
        {
            var resolvedName = context.GetCppName(t.Field.FieldType, true, true);
            int size = context.GetSize(t.Field.FieldType);
            // TODO: Ensure no conflicts with existing member names
            // Either here or during Serialization
            stateMap.TryAdd(t, new State(resolvedName, t.SafeFieldName(), size));
        }

        public void Write(CppTypeWriter writer, DllField t)
        {
            if (!stateMap.TryGetValue(t, out var st))
                throw new InvalidOperationException($"Cannot write field: {t} because it hasn't been resolved!");
            writer.WriteComment($"Offset: 0x{t.Offset:X} Size: 0x{st.size:X}");
            if (!t.Field.IsStatic)
            {
                // Only write member if it isn't static
                writer.WriteMember(st.typeName, st.name);
            }
        }
    }
